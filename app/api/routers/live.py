...
Uçlar:
  ...
  - GET /api/live/featured?limit=12[&days=7][&include_leagues=32,29][&show_all=1][&debug=1]
      ...
      * Canlı yoksa (veya azsa) ÖNÜMÜZDEKİ 'days' GÜN içinde popüler maçları getirir (varsayılan 7 gün)
...
# ------------------------------
# Featured: Popüler maçlar (live / upcoming range)
# ------------------------------
@router.get("/featured")
async def featured_matches(
  db: Annotated[Session, Depends(get_db)],
  limit: int = Query(12, ge=1, le=50, description="Toplam kart sayısı"),
  days: int = Query(7, ge=1, le=30, description="Yaklaşan maçlar için gün aralığı (varsayılan 7)"),
  ...
  # 2) UPCOMING (gerekirse 1 hafta / 'days' kadar)
  ...
  if len(live_out) < limit:
      ...
      # yakın tarihe göre ve puana göre sırala
      up_sorted = sorted(
          up_cards,
          key=lambda x: (x.get("_kick_ts", 0), -x.get("_score", 0.0))
      )
...
async def _fetch_upcoming_range(
  headers: Dict[str, str],
  weights: Dict[int, float],
  days: int = 7,
  show_all: bool = False,
  ...
  """
  Tarih aralığına göre yaklaşan popüler maçlar (bugün → +days)
  """
  now = _now_utc()
  date_from = now.strftime("%Y-%m-%d")
  date_to = (now + timedelta(days=days)).strftime("%Y-%m-%d")

  js = await _fetch_json(f"{API_BASE}/fixtures", headers, {"from": date_from, "to": date_to})
  rows = js.get("response", []) or []
  out: List[Dict] = []
  raw = len(rows)
  kept = 0
