...
def _header_nav(active: str = "", is_super: bool = False) -> str:
    ...
    for title, href, is_active in links:
        if title == "Adminler" and not is_super:  # sadece super admin
            continue
...
<style>
  :root{{--bg:#0a0b0f;--bg2:#0f0f14;--card:#111114;--muted:#b3b3bb;--text:#f5f5f5;--accent:#ff0033;--accent2:#ff4d6d;--line:#1d1d22}}
  *{{box-sizing:border-box}} body{{margin:0;background:linear-gradient(180deg,var(--bg),var(--bg2));color:var(--text);font:14px/1.55 system-ui,Segoe UI,Roboto}}
  ...
  @media(max-width:900px){{.span-6,.span-12{{grid-column:span 12}}}} @media(min-width:901px){{.span-6{{grid-column:span 6}} .span-12{{grid-column:span 12}}}}
