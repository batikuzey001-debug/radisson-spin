...
function CarouselSection({
  title, subtitle, items, trackSpeed,
}: { title: string; subtitle: string; items: Enriched[]; trackSpeed: string; }) {
  ...
  return (
    <section className="liveWrap">
      ...
      <div className="rail" style={{ ["--dur" as any]: trackSpeed }}>
        <div className="track">
          {flow.map((m, i) => <MatchCard key={`${m.id}-${i}`} m={m} />)}
        </div>
      </div>
    </section>
  );
...
function MatchCard({ m }: { m: Enriched }) {
  ...
  return (
    ...
    >
      {/* Üst şerit: Lig logo + lig adı + dakika */}
      <div className="top">
        <div className="league">
          {/* Lig LOGOSU görünür (bayrak arka planda) */}
...
.lf-wrap{min-height:100vh;background:linear-gradient(180deg,var(--bg),var(--bg2))}
...
.lgLogo{width:24px;height:24px;object-fit:contain} /* Lig LOGOSU görünür */
...
.teamcol{display:flex;flex-direction:column;align-items:center;gap:8px;min-width:0}
...
.score{
  ...
  color:#fff; text-shadow:0 0 10px rgba(255,255,255,.35),0 0 22px rgba(160,220,255,.30);
}
.score.pulse{
...
.ava{display:inline-grid;place-items:center;border-radius:999px;font-size:18px;font-weight:900;
  background:linear-gradient(180deg, rgba(255,255,255,.15), rgba(255,255,255,.05)); box-shadow:0 10px 16px rgba(0,0,0,.25)}
.ava.img{object-fit:cover;border:none;box-shadow:none;background:transparent}

@media(max-width:520px){ .card{min-width:340px;max-width:340px} .minText{font-size:20px} .score{font-size:34px} }
`;
